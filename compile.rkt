...
(define (compile-pattern p cm next)
  (match p
     ...
     (match ps
        ['() 
            (let ((empty (gensym)))
             (list (seq
             (Cmp rax (value->bits '()))
             (Je empty) ; If the list given is actually empty, then we just do nothing
             (Add rsp (* 8 (length cm))) ; Clear bindings
             (Jmp next) ; Jmp to the next case
             (Label empty)
             )
             cm))]
        [(cons p rest)
            (match (compile-pattern p (cons #f cm) next)
              [(list cari carcm)
                (match (compile-pattern (List rest) carcm next)
                  [(list cdri cdrcm)
                    (let ((ok (gensym)))
                      (list (seq  (Mov r8 rax)
                                  (And r8 ptr-mask)
                                  ...
                                  cari
                                  (Mov rax (Offset rsp (* 8 (- (sub1 (length carcm)) (length cm)))))
                                  cdri
                                  ) cdrcm))]
                )]
            )
    ...
    [(Vect ps)
      (match (compile-pattern-vector ps cm next)
        [(list cari carcm)
            (let ((isvect (gensym 'isvect))
                  (eqlens (gensym 'eqlens))
                  (empty (gensym 'empty))
                  (end (gensym 'end)))
                    (list
                      (seq
...
(define (compile-pattern-vector ps cm next)
  (match ps
  ['()
      (list (seq) cm)
  ]
  [(cons p ps)
    (match (compile-pattern p (cons #f cm) next)
      [(list cari carcm)
          (match (compile-pattern-vector ps carcm next)
          [(list cdri cdrcm)
            (let ((notempty (gensym 'notempty)))
                    (list
                        (seq
                            (Mov r9 (length ps))
                            (Cmp r9 0)
